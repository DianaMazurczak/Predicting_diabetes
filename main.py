...
# Utworzenie pliku Excel do zapisu wyników
workbook = xlsxwriter.Workbook('Wyniki_sieci_neuronowe.xlsx')
worksheet = workbook.add_worksheet()


# klasa według wzoru tutorialu pytorch
...
# link do danych: https://archive.ics.uci.edu/dataset/891/cdc+diabetes+health+indicators
...
# Model sieci bez warstw ukrytych
...
# model sieci z 1 warstwą ukrytą
...
# model sieci z 2 warstwami ukrytymi
...
# model sieci z 3 warstwami ukrytymi
...
# funkcja trenująca sieć neuronową i zwracająca ocenę modelu
def train_loop(dataloader, model, loss_fn, optimizer, batch_size, weight_for_zeros, weight_for_ones):
    ...
    for batch, sample in enumerate(dataloader):
        ...
        # uwzględnienie w procesie trenowania dysproporcji w liczbie osób chorych i zdrowych
        # gdy przewidywana jest wartość zero zmiennej sample_weight przypisywana jest wartość weight_for_zeros
        # natomiast gdy 1 - weight_for_ones
        sample_weights = torch.where(y == 0, weight_for_zeros, weight_for_ones)

        pred = model(X)
        y = y.unsqueeze(1)
        # uwzględnienie w funkcji straty wagi
        # w tym przypadku chcemy aby model zwracał większą uwagę na osoby chore
        ...
        # Funkcja sigmoid na wyjściu zapewnia wartości od 0 do 1
        # Zatem kiedy prawdopodobieństwo jest większe od 30% przewidywana jest wartość 1 (osoba jest chora)
        # W przeciwnym wypadku jest zdrowa
    ...
    # Onliczenie dokładności(acc), precyzji(precision) i czułości(recall)
    ...
    for batch_size in [32, 64, 128, 256]:
        for learning_rate in [0.1, 0.01, 0.001, 0.0001]:
            for activ in [nn.Sigmoid(), nn.ReLU(), nn.Tanh(), nn.Softplus()]:
                ...
                model = NeuralNetwork2layers(features=32, activation=activ)

                # Wykorzystanie Binary cross entropy
                loss_fn = nn.BCELoss()
                ...
                # uczenie i testowanie sieci dla podanej liczby epochs
                for t in range(epochs):
                    print(f"Epoch {t + 1}\n-------------------------------")
                    # trenowanie sieci
                    # W zestawie 16% osób to osoby chore oznaczone za pomocą jedynek, natomiast osób zdrowych
                    # jest 84% i są one oznaczone jako 0, dlatego aby wyrównać dysproporcje stsowane są wagi:
                    # dla jedynek - 0.84 a dla 0 - 0.16
                    acc_train, pr_train, rc_train = train_loop(train_dataloader, model, loss_fn, optimizer, batch_size,
                                                               0.16, 0.84)
                    # testowanie sieci
                    acc_test, pr_test, rc_test = test_loop(test_dataloader, model, loss_fn)
                    # utworzenie listy z wartościami, które mają zostać zapisane do Excela
                    output_list = \
                    ...
                    nrow += 1
    # Zapisanie całego pliku z wynikami
    workbook.close()
